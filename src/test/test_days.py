# -*- coding: utf-8 -*-
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''
### Alias : test_days.py & Last Modded : 2021.05.31. ###
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''
import os
import sys
sys.path.append(os.path.dirname(os.path.abspath(os.path.dirname(__file__))) + "/main/api")
import json
import pandas as pd
...
config = ConfigParser()
config.read(os.path.dirname(os.path.abspath(os.path.dirname(__file__))) + "/main/api/key.config")  # https://codechacha.com/ko/python-examples-get-working-directory/
...
if __name__ == '__main__':
    queryParams = '?' + urlencode({quote_plus('ServiceKey'): API_KEY,
                                   ...
                                   quote_plus('startDt'): '20201230',
                                   quote_plus('endDt'): '20210607',
    ...
    print(df)
    df.to_csv(os.path.dirname(os.path.realpath(__file__)) + "/test_days.csv", sep=',', encoding='utf-8')
